  ...
  return (
    <section className="w-full max-w-5xl mx-auto px-4 py-16">
      ...
      <div className="mt-8 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {allArts.map((filename) => {
          const desc = descriptions[filename] || "";
          const label = filename.replace(/^\d+\.\s*/, "").replace(/\.[^.]+$/, "");
          return (
            <div key={filename} className="flex flex-col gap-2">
              <div
                className="relative aspect-square rounded-xl overflow-hidden cursor-pointer border border-white/10 hover:border-cyan-400/50 transition-all duration-300 hover:shadow-[0_0_20px_rgba(34,211,238,0.2)] group"
